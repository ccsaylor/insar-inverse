...
class DataStore:
    #indicates with bool whether to expect data in each direction
    #columns must always be in the order: x, y, dx, dy, dz
    #if a column is omitted, put False for the corresponding parameter
    #example: if fitting dx and dz, the parameters are True, False, True
    def __init__(self, usedx, usedy, usedz):
        self.usedx = usedx
        self.usedy = usedy
        self.usedz = usedz
    
    #imports the data to be fitted
    #expected format is tab separated: x, y, dx, dy, dz
    ...
    #calculates the greens functions to determine effect of sources on the displacement
    #these are used to calculate the total ground displacement generated by a model
    #reads point source parameters from the input paramfile.ini
    ...
    #imports a pre-calculated model
    #used to plot the model without having to rerun it
    def ImportBeta(self, filename):
        points = np.genfromtxt(filename, dtype = 'float', delimiter='\t')
        
        self.Beta = np.asarray(points[:,6])
        
    #uses multiple linear regression to solve for the point source moments
    #only works if seismic moment is the only parameter that changes
    #if other parameters change, the problem is nonlinear
    ...
    #function minimized if using LMFit as the solver
    #LMFit can be used for nonlinear problems
    ...
    #uses the package LMFit to solve for the point source moments
    #can solve nonlinear problems
    #slows down greatly for large numbers of parameters
    ...
    #saves the model resulting from the fit
    ...
    #uses LMFit to solve for the point source moments using an initial guess
    #helps to reduce processing time for larger numbers of parameters
    #if no guess, uses LMFit normally
...
class Plotter:
    ...
    #plots the data and model side by side for each direction used in the fit: dx, dy, and/or dz
    ...
    #plots the distribution of point sources resulting from the fit
    #darker colors indicate greater seismic moment (or more slip at that source)
    def PlotModelPoints(self):
