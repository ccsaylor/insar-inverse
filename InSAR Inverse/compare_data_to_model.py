...
files = ['fifteen_total_displacement_binned_converted.txt', 'fifteen5_total_displacement_binned_converted_model_data.txt','fifteen5_total_displacement_binned_converted_residual.txt']
...
for filename in files:
    ...
    plt.colorbar(test)
    diff = len(set(Y))/len(set(X))*(X.max()-X.min())
    #ax.auto_scale_xyz([X.min(), X.max()], [Y.min() - diff, Y.max() + diff], [-0.02, 0.02]) 

    i += 1


resfile = np.genfromtxt('fifteen5_total_displacement_binned_converted_residual.txt', dtype = 'float', delimiter='\t')
